// WebSocket message handlers (extracted from server.js)

const { broadcastToZone, safeSend } = require('./websocket/broadcast');
const {
  normalizeSafeString,
  isValidRoomId,
  isValidPlayerId,
...
/**
 * Create all WebSocket handlers bound to shared dependencies.
 * Returns a handler map: { [messageType]: (ws, data) => void }
 */
function createHandlers({ rooms, pool, currency, wss, loadZoneData, ZoneSession, TankZoneSession, MAX_PARTY_SIZE, ENEMY_KILL_REWARD, DEATH_PENALTY_COINS }) {
  ...
  function broadcastRoomList() {
    ...
    wss.clients.forEach(client => {
      if (client.readyState === 1 /* WebSocket.OPEN */ && !client.roomId) {
  ...
  // Buffer player_state updates per room+zone and flush at 20 Hz
  // Reduces per-message overhead when multiple players share a zone

  const STATE_FLUSH_INTERVAL_MS = 50; // 20 Hz
  // Map<roomId, Map<zoneId, Map<playerId, state>>>
  const stateBatches = new Map();

  function bufferPlayerState(roomId, zoneId, playerId, state) {
    if (!stateBatches.has(roomId)) stateBatches.set(roomId, new Map());
    const roomBatch = stateBatches.get(roomId);
    if (!roomBatch.has(zoneId)) roomBatch.set(zoneId, new Map());
    roomBatch.get(zoneId).set(playerId, state);
  }

  const flushTimer = setInterval(() => {
    for (const [roomId, zoneBatches] of stateBatches) {
      const room = rooms.getRoom(roomId);
      if (!room) { stateBatches.delete(roomId); continue; }

      for (const [zoneId, playerStates] of zoneBatches) {
        if (playerStates.size === 0) continue;

        const updates = [];
        for (const [playerId, state] of playerStates) {
          updates.push({ playerId, state });
        }
  ...
  function handlePlayerUpdate(ws, data) {
    ...
    // Buffer for batched broadcast instead of sending immediately
    bufferPlayerState(ws.roomId, ctx.player.zone, ws.playerId, data.state);
  ...
  function handleTankCrateDamage(ws, data) {
    ...
    const ctx = getZoneSession(ws);
    if (!ctx || !ctx.session.applyCrateDamage) return;
    ctx.session.applyCrateDamage(data.crateId, data.damage);
  }
  ...
  // Handler map: message type -> handler function
  const MESSAGE_HANDLERS = {
    join_room:         handleJoinRoom,
    leave_room:        handleLeaveRoom,
    player_update:     handlePlayerUpdate,
    game_start:        handleGameStart,
    zone_enter:        handleZoneEnter,
    enemy_damage:      handleEnemyDamage,
    list_rooms:        handleListRooms,
    player_death:      handlePlayerDeath,
    player_fire:       handlePlayerFire,
    player_chat:       handlePlayerChat,
    tank_restart:      handleTankRestart,
    tank_crate_damage: handleTankCrateDamage,
  };

  return { MESSAGE_HANDLERS, handleDisconnect };
}
